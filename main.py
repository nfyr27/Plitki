...
def play_note():
    ...
    if next_note > 0 :
        sound.fadeout(1000)
    sound = pg.mixer.Sound(f"Sounds/{playing_song.notes[next_note]}.ogg")
    sound.play()
    next_note += 1
...
while True:
    ...
    if mode == "menu":
        ...
        for song in Song.songs:
            ...
            screen.blit(song.text, song.rect)

    if mode == "play":
        if is_play:  # только если сейчас идёт игра - нет проигрыша или победы
            is_time = time.time() - timer >= playing_song.duration[created_notes] * playing_song.interval
            if created_notes < len(playing_song.notes) and is_time:
